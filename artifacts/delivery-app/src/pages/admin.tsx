...
export default function Admin() {
  ...
  const onWhatsApp = (order: Order) => {
    ...
    let text = tpl
      ...
      .replace("{area}", (order as any).deliveryArea || "غير محدد")
      .replace("{deliveryFee}", String((order as any).deliveryFee || 0))
      .replace("{paymentMethod}", (order as any).paymentMethod === "cash" ? "عند الاستلام" : (order as any).paymentMethod === "bank_transfer" ? "تحويل مصرفي" : "غير محدد")
